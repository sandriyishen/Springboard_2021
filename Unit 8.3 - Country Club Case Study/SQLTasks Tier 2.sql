...
Facities columns: facid, name, membercost, guestcost, initialoutlay, monthlymaintenance
...
SELECT DISTINCT f.name AS court_name, CONCAT(m.surname, ' ', m.firstname) AS member_name
FROM Bookings b
...
SELECT b.bookid, f.name AS facility_name, CONCAT(m.surname, ' ', m.firstname) as member_name,
...
SELECT sub.bookid, CONCAT(m.surname, ' ', m.firstname) as member_name, sub.cost
...
SELECT r.name, SUM(r.revenue) as total_revenue
FROM (
  SELECT f.name,
    CASE WHEN b.memid = 0 THEN f.guestcost * b.slots
...
SELECT f.name as facility,
  ...
  ROUND(SUM(b.slots)/2, 1) as hours_booked
FROM Bookings b
LEFT JOIN Facilities f ON b.facid = f.facid
LEFT JOIN Members m ON b.memid = m.memid
WHERE b.memid <> 0
